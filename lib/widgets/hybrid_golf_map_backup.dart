...
class _HybridGolfMapState extends State<HybridGolfMap> {
  ...
  void initState() {
    ...
    // Set initial rotation after a short delay to ensure map is ready
  ...
  void _centerOnCurrentHole() {
    ...
    // Since we want green pointing north (0 degrees), we need to rotate by negative bearing
    final targetRotation = -bearingDegrees;
    
    // Store the rotation for counter-rotating text
    _currentMapRotation = targetRotation;
    
    // Set rotation first (rotation is relative to center)
    _mapController.rotate(targetRotation);
    
    // Calculate distance for offset calculations
    ...
    // Try to move - these methods might fail silently if not available
    try {
      if (_mapController.move != null) {
  ...
  Future<void> _safeMapMove(latlong.LatLng center) async {
    // Alternative approach when move() isn't available
    await Future.delayed(const Duration(milliseconds: 100));
    if (mounted) {
      setState(() {
        // This will rebuild the widget with new center
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final currentHoleData = widget.currentHole <= widget.courseData!.holes.length
        ? widget.courseData!.holes[widget.currentHole - 1]
        : null;
      ...
      children: [
        ...
        if (currentHoleData != null)
          Positioned(
            top: 16,
            left: 16,
            child: _buildHoleInfoCard(currentHoleData),
          ),
      ],
    );
  }
  ...
  void _handleMarkerDrag(HoleData hole, int pointIndex, DraggableDetails details) {
    ...
    final lat = bounds.north - (localPosition.dy / size.height) * latRange;
    final lng = bounds.west + (localPosition.dx / size.width) * lngRange;
    
    // Update the marker position
    _updatePreferredPathPoint(hole, pointIndex, latlong.LatLng(lat, lng));
  }
  ...
  List<Marker> _buildSimpleMarkers(HoleData hole) {
    ...
    if (preferredPath != null && preferredPath.length > 2) {
      ...
      for (int i = 1; i < preferredPath.length - 1; i++) {
        ...
        // Calculate distance from tee to this landing area
  ...
  Widget _buildHoleInfoCard(HoleData hole) {
    return Card(
      elevation: 4,
      child: Padding(
